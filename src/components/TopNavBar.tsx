  ...
  return (
    <nav className=' bg-white dark:bg-slate-300 h-[60px] md:px-20 px-4 flex justify-between items-center sticky top-0 border-b-1 border-slate-800 z-50'>
       <Link href={'/'} className={'inline-block'}>
        <Image src={'/images/logot2.png'} width={200} height={200} quality={100} alt="logo" />
      </Link>
      <div className=' hidden  md:block items-center text-slate-800'>
       {navigation.map((item) => (
        <Link href={item.sectionId} key={item.sectionId} className=' cursor-pointer hover:text-yellow-800 hover:translate-y-2 hover:py-1 hover:transition-all mr-5'>{item.title}</Link>
       ))}
      </div>
